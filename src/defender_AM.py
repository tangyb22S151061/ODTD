...
def train_defender():
    ...
    train_oe_loader, test_oe_loader = get_dataset(
        args.dataset_oe, args.batch_size, augment=True)
    ...
    if args.opt == 'sgd':
        ...
        if model_poison is not None:
            opt_poison = optim.SGD(model_poison.parameters(
            ), lr=args.lr_tgt, momentum=0.9, weight_decay=5e-4)
        ...
        if model_poison is not None:
            opt_poison = opt = optim.Adam(model.parameters(), lr=1e-5)
    else:
        sys.exit('Invalid optimizer {}'.format(args.opt))

    best_acc = 0
    for epoch in range(args.epochs):
        train_loss, train_acc = train_epoch_AM(model, args.device, train_loader, train_oe_loader,
                                               opt, args, model_poison=model_poison, optimizer_poison=opt_poison, disable_pbar=False)
        test_loss, test_acc = test_AM(
            model, args.device, test_loader, model_poison)
