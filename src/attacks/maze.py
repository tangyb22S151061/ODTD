...
from utils import test, blackbox, defense_output
...
def maze_original(args, T, S, train_loader, test_loader, tar_acc, T_posion=None):
    ...
    if args.opt == "sgd":
        ...
        optS = optim.Adam(S.parameters(), lr=1e-7)
    ...
    pbar = tqdm(range(1, iter + 1), ncols=80,
                disable=args.disable_pbar, leave=False)
...
def maze(args, T, S, train_loader, test_loader, tar_acc, T_posion=None):
    ...
    savedir = "{}/{}/{}/clone1/{}/without def/{}/".format(args.logdir, args.dataset, args.model_tgt, args.dataset_sur, args.model_clone)
...
def maze_new(args, T, T_ft, D_ft, rank_map, S, train_loader, test_loader, tar_acc, T_posion=None):
    ...
    if args.alpha_gan > 0:
        ...
        if T_posion is not None:
            ...
            # Tout = T(x_seed)
            Tout = defense_output(x_seed, T, T_ft, D_ft, rank_map)
        batch = [
            (a, b)
            for a, b in zip(x_seed.cpu().detach().numpy(), Tout.cpu().detach().numpy())
        ]
        ...
        for g in range(args.iter_gen):
            ...
            if args.white_box:
                if T_posion is not None:
                    ...
                    # Tout = T(x)
                    Tout = defense_output(x, T, T_ft, D_ft, rank_map)
                Sout = S(x)
                lossG_dis = -kl_div_logits(args, Tout, Sout)
                (lossG_dis).backward(retain_graph=True)
            else:

                tmp1, tmp2, tmp3 = zoge_backward_new(args, x_pre, x, S, T, T_ft, D_ft, rank_map)
    ...
    savedir = "{}/{}/{}/clone1/{}/with def/{}/".format(args.logdir, args.dataset, args.model_tgt, args.dataset_sur, args.model_clone)
    if not os.path.exists(savedir):
        os.makedirs(savedir)
