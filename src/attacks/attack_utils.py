...
from utils import defense_output
...
def zoge_backward_new(args, x_pre, x, S, T, T_ft, D, rank_map):
    for p in S.parameters():
        p.requires_grad = False
    ...
    with torch.no_grad():
        ...
        Sout = S(x)
        # Tout = T(x)
        Tout = defense_output(x, T, T_ft, D, rank_map)
        ...
        for _ in range(args.ndirs):
            ...
            Sout = S(x_mod)
            # Tout = T(x_mod)
            Tout = defense_output(x_mod, T, T_ft, D, rank_map)
    ...
    Sout = S(x_det)
    # Tout = T(x_det)
    Tout = defense_output(x_det, T, T_ft, D, rank_map)
    if Tout == 'Detected by PRADA':
        return False, False, False
